...
def onTrail():
	...
	if not os.path.exists('C:/Python/record/Record'):
		Record = str(random.randint(100, 1000)) + time.strftime(':%Y%m%d%H%M%S', time.localtime(time.time()))
		with open('C:/Python/record/Record', 'w') as file:
			file.write(base64.b64encode(Record))
	...
	if yearNow == 2016 and monthNow == 9 and dayNow <= 13 and dayNow >= 1:
		return True
	elif yearNow == 2016 and monthNow == 8 and dayNow == 30:
		return True
	elif yearNow == 2016 and monthNow == 8 and dayNow == 31:
		return True
	else:
		return False
